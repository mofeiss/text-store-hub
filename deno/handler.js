...
async function authenticate(request, env) {
  ...
  var expected = await computeAuthToken(env.adminPassword);
...
async function handleLogin(request, env) {
  ...
  var token = await computeAuthToken(env.adminPassword);
...
async function handlePublicFile(pathname, env) {
  ...
  var metaJson = await env.store.get('meta:index');
  ...
  var content = await env.store.get('content:' + file.id);
...
async function handleCreateFile(request, env) {
  ...
  await env.store.put('meta:index', JSON.stringify(metadata));
  await env.store.put('content:' + id, content);
...
async function handleGetFile(id, env) {
  ...
  var content = await env.store.get('content:' + id);
...
async function handleDeleteFile(id, env) {
  ...
  await env.store.put('meta:index', JSON.stringify(newMeta));
  await env.store.delete('content:' + id);
...
async function handleExport(env) {
  ...
  for (var i = 0; i < metadata.length; i++) {
    var m = metadata[i];
    var content = await env.store.get('content:' + m.id);
    files.push({
      id: m.id,
      filename: m.filename,
      title: m.title,
