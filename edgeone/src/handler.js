...
export async function onRequest(context) {
  ...
  var env = context.env || {};
  var storeKv = env.TEXT_STORE_KV || globalThis.TEXT_STORE_KV;
  var adminPassword = env.ADMIN_PASSWORD || globalThis.ADMIN_PASSWORD;

  var adaptedEnv = {
    TEXT_STORE_KV: storeKv,
  ...
  if (pathname.startsWith('/f/')) {
    return handlePublicFile(pathname, adaptedEnv);
  }

  // Login/logout - no auth needed
  if (pathname === '/api/login' && method === 'POST') {
    return handleLogin(request, adaptedEnv);
  }
  ...
  if (pathname.startsWith('/api/')) {
    if (!(await authenticate(request, adaptedEnv))) {
      return errorResponse('Unauthorized', 401);
    }
    return handleApi(pathname, method, request, adaptedEnv);
  }
